...
from database import insert_button_or_relay_to_db

# Creating Client name - should be unique 
global clientname, CONNECTED
CONNECTED = False
r=random.randrange(1,10000000)
clientname="IOT_SmartHeat-Id-"+str(r)

button_topic = 'IoT/SmartHeat/Button/sts'
...
class ConnectionDock(QDockWidget):
    ...
    def push_button_click(self):
        ...
        if CONNECTED:
            if ON:
                self.mc.publish_to(self.ePublisherTopic.text(), '"value":0')
                date = datetime.datetime.now().strftime(dateFromat)
                insert_button_or_relay_to_db(database,tablename=button_table,value1=date,value2=0)
                self.ePushtbtn.setStyleSheet("background-color: grey")
                ON = False
            else:
                self.mc.publish_to(self.ePublisherTopic.text(), '"value":1')
                date = datetime.datetime.now().strftime(dateFromat)
                insert_button_or_relay_to_db(database,tablename=button_table,value1=date,value2=1)
